...
defaultTheme = 'Stockholm',
    queryString = bryntum.gantt.BrowserHelper.queryString,
...
disableRootCause = ['bigdataset', 'csp'],
  ...
  return pathName.endsWith(path) || Boolean(pathName.match(path + 'index.*html$'));
    ...
    isOnline = location.host.includes('bryntum.com'),
...
var Shared = /*#__PURE__*/function (_bryntum$gantt$Locali) {
  ...
  function Shared() {
    ...
    if (reset) {
      bryntum.gantt.BrowserHelper.removeLocalStorageItem('b-example-language');
      bryntum.gantt.BrowserHelper.removeLocalStorageItem('b-example-theme');
    ...
    if (themeInfo && themes[themeInfo.name.toLowerCase()]) {
      var theme = bryntum.gantt.BrowserHelper.queryString.theme || bryntum.gantt.BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme; // Apply default theme first time when the page is loading
    ...
    me.localeManager.on('locale', function (localeConfig) {
      return bryntum.gantt.BrowserHelper.setLocalStorageItem('b-example-language', localeConfig.locale.localeName);
    }); // Apply default locale first time when the page is loading

    me.localeManager.applyLocale(bryntum.gantt.BrowserHelper.getLocalStorageItem('b-example-language') || bryntum.gantt.LocaleManager.locale.localeName, false, true); //}
    ...
    // const
    //     positionMode    = bryntum.gantt.BrowserHelper.queryString.position || 'translate',
    //     testPerformance = bryntum.gantt.BrowserHelper.queryString.testPerformance,
    //     rowScrollMode   = bryntum.gantt.BrowserHelper.queryString.rowScrollMode || 'move';
    //
    ...
    if (!isOnline && !bryntum.gantt.VersionHelper.isTestEnv) {
  ...
  _createClass(Shared, [{
    ...
    value: function insertHeader() {
      ...
      var tools = document.getElementById('tools') || document.body,
          fullscreenButton = bryntum.gantt.WidgetHelper.createWidget({
        type: 'button',
    ...
    value: function loadDescription() {
      ...
      bryntum.gantt.AjaxHelper.get(url, {
        ...
        var localeValue = me.localeManager.locale.localeName,
            storedLocaleValue = bryntum.gantt.BrowserHelper.getLocalStorageItem('b-example-language'),
        ...
        button.menu = {
          ...
          items: [{
            ...
            editable: false,
            value: bryntum.gantt.StringHelper.capitalizeFirstLetter(bryntum.gantt.BrowserHelper.getLocalStorageItem('b-example-theme') || defaultTheme),
            items: themes,
        ...
        bryntum.gantt.GlobalEvents.on({
          theme: function theme(_ref7) {
            ...
            bryntum.gantt.BrowserHelper.setLocalStorageItem('b-example-theme', _theme);
    ...
    value: function initRootCause() {
      ...
      if ((isOnline || isRootCauseReplay) && !isExcluded && !bryntum.gantt.VersionHelper.isTestEnv) {
    ...
    value: function startRootCause() {
      var appIds = {
        ...
        gantt: '9df03cbdc00b455de8bfe58d831e6fd76cc8881e'
          ...
          version = bryntum.gantt.VersionHelper.getVersion(productName),
      ...
      window.logger = new RC.Logger({
        ...
        recorderConfig: {
          ...
          ignoreCssClasses: ['focus', 'hover', 'dirty', 'resizable', 'committing', 'b-active', 'b-sch-terminals-visible']
        },
        version: version,
    ...
    value: function performVersionCheck() {
      var lastCheck = bryntum.gantt.BrowserHelper.getLocalStorageItem('b-latest-version-check-timestamp'),
      ...
      bryntum.gantt.AjaxHelper.get("https://bryntum.com/latest/?product=".concat(checkProductName), {
        parseJson: true,
        credentials: 'omit'
      }).then(this.notifyIfLaterVersionExists).catch(function () {});
      bryntum.gantt.BrowserHelper.setLocalStorageItem('b-latest-version-check-timestamp', Date.now());
    }
  }, {
    ...
    value: function notifyIfLaterVersionExists(response) {
      ...
      if (latestVersion && bryntum.gantt.VersionHelper.checkVersion(productName, latestVersion, '<')) {
        var toast = bryntum.gantt.Toast.show({
        ...
        toast.element.addEventListener('click', function () {
          ...
          bryntum.gantt.BrowserHelper.setLocalStorageItem('b-latest-version-check-timestamp', inTwoWeeks);
        });
      }
    } // endregion
...
var CodeEditor = /*#__PURE__*/function (_bryntum$gantt$Popup) {
              ...
              case 23:
                ...
                bryntum.gantt.DomHelper.forEachSelector('.b-widget', function (element) {
                  ...
                  if (widget && !widget.isDestroyed && !widget.owner && !renderedWidgets.has(widget)) {
                    try {
                      widget.destroy();
                    } catch (e) {// We might be in a case where a misconfigured bryntum.gantt.Widget throws an exception mid-setup
                    }
                  }
                });
